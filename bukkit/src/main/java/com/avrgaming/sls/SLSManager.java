...
public class SLSManager implements Runnable {
    ...
    public static void sendHeartbeat() {
        try {
            ...
            String message =
                    gen_id + ";" + serverName + ";" + serverDescription + ";" + serverTimezone + ";" + serverAddress +
                    ";" + Bukkit.getOnlinePlayers().length + ";" + Bukkit.getMaxPlayers() + ";" + getParsedVersion();

            try {
                if (Boolean.valueOf(CivSettings.getStringBase("debug_heartbeat"))) {
            ...
            DatagramPacket packet =
                    new DatagramPacket(message.getBytes(), message.toCharArray().length, address, 25580);
            ...
            CivLog.error("Couldn't IP address to SLS service. If you're on a LAN with no internet access, " +
                         "disable SLS in the CivCraft config.");
            //e.printStackTrace();
        }
    }
