    ...
    public void run() {
        ...
        if (resident == null) {
            ...
            try {
                ...
                TaskMaster
                        .syncTask(new PlayerKickBan(player.getName(), true, false, "You have an invalid name. Sorry."));
                return;
            }
            ...
            CivMessage.send(resident, CivColor.LightGray + "You have a PvP timer enabled for " + mins +
                                      " mins. You cannot attack or be attacked until it expires.");
        ...
        if (War.isWarTime() && War.isOnlyWarriors()) {
            if (player.isOp() || player.hasPermission(CivSettings.MINI_ADMIN)) {
                ...
                TaskMaster.syncTask(new PlayerKickBan(player.getName(), true, false,
                                                      "Only players in civilizations at war can connect right now. " +
                                                      "Sorry."));
                return;
            }
        }
                ...
                if (War.isWarTime() && status.equals(Relation.Status.WAR)) {
                    ...
                    if (resident.getLastOnline() < War.getStart().getTime()) {
                        ...
                        CivMessage.send(resident, CivColor.LightGray +
                                                  "You've been teleported back to your home since you've logged into " +
                                                  "enemy during WarTime.");
                    }
                }

                CivMessage.sendCiv(cc.getCiv(), color + player.getDisplayName() + "(" + relationName + ") has " +
                                                "logged-in to our borders.");
            }
        }
        ...
        try {
            ...
            } else if (Boolean.valueOf(CivSettings.getString(CivSettings.perkConfig, "system.free_admin_perks")) &&
                       player.hasPermission(CivSettings.MINI_ADMIN)) {
                resident.giveAllFreePerks();
            }
        ...
        try {
            ...
            PlatinumManager.givePlatinumDaily(resident, CivSettings.platinumRewards.get("loginDaily").name,
                                              CivSettings.platinumRewards.get("loginDaily").amount,
                                              "Welcome back to CivCraft! Here is %d for logging in today!");


            ArrayList<SessionEntry> deathEvents =
                    CivGlobal.getSessionDB().lookup("pvplogger:death:" + resident.getName());
            if (deathEvents.size() != 0) {
                CivMessage.send(resident, CivColor.Rose + CivColor.BOLD +
                                          "You were killed while offline because you logged out while in PvP!");
        ...
        if (EndConditionDiplomacy.canPeopleVote()) {
            CivMessage.send(resident, CivColor.LightGreen +
                                      "The Council of Eight is built! Use /vote to vote for your favorite " +
                                      "Civilization!");
        }
    }
}
