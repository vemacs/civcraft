...
public class PlatinumManager implements Runnable {
        ...
        class AsyncTask implements Runnable {
            ...
            public void run() {
                ...
                if (entries.size() == 0) {
                    ...
                    CivGlobal.getSessionDB().global_update(entries.get(0).request_id, getDailyKey(resident, ident),
                                                           "" + now.getTimeInMillis());
                }
    ...
    public static void giveManyPlatinumDaily(LinkedList<Resident> residents, String ident, Integer plat,
                                             String reason) {
        if (!isEnabled()) return;
        ...
        class GiveManyPlatinumDailyAsyncTask implements Runnable {
            ...
            public GiveManyPlatinumDailyAsyncTask(LinkedList<Resident> residents, Integer plat, String reason,
                                                  String ident) {
            ...
            public void run() {
                for (Resident resident : residents) {
                    ArrayList<SessionEntry> entries =
                            CivGlobal.getSessionDB().global_lookup(getDailyKey(resident, ident));
                    Calendar now = Calendar.getInstance();
    ...
    public void updatePendingPlatinum() {
        ...
        HashMap<String, Queue<PendingPlatinum>> readdUs = new HashMap<>();
        for (String key : pendingPlatinum.keySet()) {
            Queue<PendingPlatinum> pendingList = pendingPlatinum.get(key);
            Queue<PendingPlatinum> newQueue = new LinkedList<>();
            ...
            while (pending != null) {
                ...
                try {
                    ...
                    reason = CivColor.LightGreen +
                             reason.replace("%d", CivColor.Yellow + plat + " Platinum" + CivColor.LightGreen);
                    ...
                    if (!warnedPlayers.contains(resident.getName())) {
                        CivMessage.sendError(resident, "Aww man! You've earned " + CivColor.Yellow + pending.amount +
                                                       " Platinum" + CivColor.Rose +
                                                       " but your in-game name is not currently verified!");
                        CivMessage.sendError(resident, "Go to " + CivColor.Yellow + url + CivColor.Rose + " and " +
                                                       "verify first! We'll hold on to it until the server reboots.");
    ...
    public static boolean isEnabled() {
        try {
            return Boolean.valueOf(CivSettings.getString(CivSettings.perkConfig, "system.enabled"));
